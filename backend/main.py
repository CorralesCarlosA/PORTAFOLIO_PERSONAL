...
SECRET_KEY = "tu_clave_secreta_super_segura_para_jwt"
ALGORITHM = "HS256"
...
class Profile(Base):
    ...
    headline = Column(String, default="Ingeniería de sistemas | Desarrollador Python & Backend | APIs REST ꞏ Automatización IA")
    bio = Column(Text, default="Especialista en desarrollo backend con Python, diseño y construcción de APIs RESTful de alto rendimiento, arquitecturas escalables, gestión de bases de datos relacionales y automatización inteligente con Inteligencia Artificial.")
    ...
    github_url = Column(String, default="https://github.com/CorralesCarlosA?tab=repositories")
    ...
    skills = Column(Text, default="Python, FastAPI, Django, APIs REST, PostgreSQL, SQLite, Docker, Git, Automatización IA, React Native, JavaScript")
...
def get_projects(db: Session = Depends(get_db)):
    ...
    projects = db.query(Project).filter(Project.is_active == True, Project.is_idea == False).all()
    result = []
    for p in projects:
        result.append({
            "id": p.id,
            ...
            "is_idea": False,
            "media": [{"id": m.id, "file_path": m.file_path, "media_type": m.media_type} for m in p.media],
            "technologies": [{"id": t.id, "name": t.name, "icon_url": t.icon_url} for t in p.technologies],
            "comments": [{"id": c.id, "author": c.author, "content": c.content} for c in p.comments]
        })
    return result
...
def get_ideas(db: Session = Depends(get_db)):
    ...
    ideas = db.query(Project).filter(Project.is_active == True, Project.is_idea == True).all()
    result = []
    for p in ideas:
        result.append({
            "id": p.id,
...
def add_comment(project_id: int, author: str = Form(...), content: str = Form(...), db: Session = Depends(get_db)):
...
def send_private_message(project_id: int, sender_email: str = Form(...), message: str = Form(...), db: Session = Depends(get_db)):
    pm = PrivateMessage(project_id=project_id, sender_email=sender_email, message=message)
    db.add(pm)
    db.commit()
    return {"status": "Mensaje enviado"}

@app.post("/contact")
def send_general_contact(sender_email: str = Form(...), message: str = Form(...), db: Session = Depends(get_db)):
...
def admin_get_all_projects(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_private_messages(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def upload_media(
    ...
    media = Media(project_id=project_id, file_path=f"/uploads/{filename}", media_type=media_type)
    db.add(media)
    db.commit()
